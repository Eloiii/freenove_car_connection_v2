...
from g2o_optimizer import bundle_adjustment, pose_optimization
...
class SLAM:
    ...
    def run_video(self, process_n):
        ...
        # points = get_poses(),
        points = np.array([(-np.dot(frame.pose[:3, :3].T, frame.pose[:3, 3].T)) for frame in self.map.frames])
    ...
    def match_frames(self, desc1, desc2):
        ...
        return np.array(frame1_match_idx), np.array(frame2_match_idx), matches, mask
    ...
    def track_last_keyframe(self, curr_frame):
        ...
        cv.imwrite('im1.jpg', curr_frame.image)
        cv.imwrite('im2.jpg', last_keyframe.image)

        match_idx_curr, match_idx_kf, matches, mask = self.match_frames(curr_frame.desc,
                                                                        last_keyframe.desc[
                                                                            last_keyframe.desc_filtered_idx])
        matched = cv.drawMatchesKnn(curr_frame.image,
                                    curr_frame.real_kp,
                                    last_keyframe.image,
                                    last_keyframe.real_kp,
                                    matches[100:110],
                                    None,
                                    flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)

        cv.imwrite('mattches.jpg', matched)
        ...
        # pts = pts[world_points_idx[idx_kf_inliers]]
        # pose_optimization(curr_frame, self.K, pts)
    ...
    def create_new_map_points(self, curr_kf_pose):
        ...
        for frame_id in connected_views_ids:
            ...
            match_idx_1, match_idx_2, matches, mask = self.match_frames(unmatched_desc1, unmatched_desc2)
    ...
    def iter(self, img, curr_frame_idx):
        if self.state == 'NO_FRAME':
            ...
            match_idx_curr, match_idx_ref, matches, mask = self.match_frames(curr_frame.desc, ref_frame.desc)
